...
class Benchmark:
    ...
    def _evaluation_query(self):
        ...
        """
        converted_cons = str(tuple(self._constituents))
        query = F'SELECT price FROM prices WHERE type="{self._evaluation_type}" AND date="{self.evaluation_date}" ' \
                F'AND uuid IN {converted_cons}'  # query should work but will need to test to make sure
        return query
    ...
    def evaluate_constituents(self, evaluation_table_curr):
        ...
        None
        """
        evaluation_query = self._evaluation_query()
        results = evaluation_table_curr.execute(evaluation_query).fetchall()
        if len(results) > 0:
            self._values = [entry[0] for entry in results]
        else:
            warnings.warn(F'No values found with query {evaluation_query}', UserWarning)
